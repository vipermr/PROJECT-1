...
// --- Fixed Background URL for match1 ---
...
const avatarRelativeSize = 0.35;   // Avatars will be 35% of the BACKGROUND's height.
const avatarRelativeY = 0.4;      // Avatars' top edge will be 40% down from the BACKGROUND's top edge.
const avatarRelativeXGap = 0.04;  // Avatars' gap from left/right edges will be 4% of the BACKGROUND's width.
...
module.exports = {
  ...
  onStart: async function ({ api, event, args, usersData, threadsData }) {
    ...
    const pathImg = __dirname + "/tmp/match1_background.png"; // Unique name to avoid conflicts
    ...
    const intervalMessages = {
        ...
        "80": "80%! Excellent synergy! You'sre practically a dynamic duo. Prepare for adventures and mutual understanding!",
    ...
    // --- 4. Select Fixed Background Image ---
    const selectedBackground = FIXED_BACKGROUND_URL; // Using the fixed URL here

    // --- 5. Image Processing and Sending Message ---
    try {
      console.log("[Match1 Cmd] Attempting to download avatars and fixed background...");
      ...
      console.log(`[Match1 Cmd] Avatar 1 saved. Size: ${fs.existsSync(pathAvt1) ? fs.statSync(pathAvt1).size : '0'} bytes.`);
      console.log(`[Match1 Cmd] Avatar 2 saved. Size: ${fs.existsSync(pathAvt2) ? fs.statSync(pathAvt2).size : '0'} bytes.`);
      console.log(`[Match1 Cmd] Background saved. Size: ${fs.existsSync(pathImg) ? fs.statSync(pathImg).size : '0'} bytes.`);
      
      console.log("[Match1 Cmd] Attempting to load images onto canvas...");

      const baseImage = await loadImage(pathImg);
      const baseAvt1 = await loadImage(pathAvt1);
      const baseAvt2 = await loadImage(pathAvt2);

      console.log("[Match1 Cmd] Images loaded successfully. Proceeding to draw.");
      ...
      console.log("[Match1 Cmd] Composite image saved. Sending to chat.");
      ...
      console.error(`[Match1 Cmd] Error during command execution:`, error);
      ...
      fs.remove(pathAvt1).catch(err => console.error("[Match1 Cmd] Failed to remove temp avatar 1:", err));
      fs.remove(pathAvt2).catch(err => console.error("[Match1 Cmd] Failed to remove temp avatar 2:", err));
    }
  },
};
